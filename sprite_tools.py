...
class SpriteSheet(object):
    ...
    def __init__(self, img_path, sheet_size, frame_num, scale=1):
    ...
    def load_image_file(self):
        ...
        if self.scale != 1:
            new_width = self.sheet_img.get_width() * self.scale
            new_height = self.sheet_img.get_height() * self.scale
            self.sheet_img = pygame.transform.scale(self.sheet_img, (int(new_width), int(new_height)))
...
class Sprite(object):
    ...
    def draw(self, surf):
        ...
        x = self.x_pos - frame_to_draw.get_width()/2
        y = self.y_pos - frame_to_draw.get_height()/2
        surf.blit(frame_to_draw, (int(x), int(y)))
